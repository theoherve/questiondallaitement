 ...
 * « Mon bébé ne fait pas ses nuits » (accompagnement_sections.sales_hook).
 *
 ...
 * data/sales-hook-nuits.mjs. Un chapitre sans correspondance est signalé,
 * jamais deviné.
 *
 *   node --env-file=.env.local scripts/apply-sales-hook-nuits.mjs          # dry-run
 *   node --env-file=.env.local scripts/apply-sales-hook-nuits.mjs --apply
 */

import { createClient } from "@supabase/supabase-js";
import { SALES_HOOKS_NUITS } from "./data/sales-hook-nuits.mjs";

const APPLY = process.argv.includes("--apply");
const SLUG = "mon-bebe-ne-fait-pas-ses-nuits";
...
for (const entry of SALES_HOOKS_NUITS) {
...
console.log(`Correspondances trouvées : ${matched.length}/${SALES_HOOKS_NUITS.length}\n`);
