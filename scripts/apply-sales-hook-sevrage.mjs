 ...
 * « Je souhaite sevrer mon bébé » (accompagnement_sections.sales_hook).
 *
 ...
 * data/sales-hook-sevrage.mjs. Un chapitre sans correspondance est signalé,
 * jamais deviné.
 *
 *   node --env-file=.env.local scripts/apply-sales-hook-sevrage.mjs          # dry-run
 *   node --env-file=.env.local scripts/apply-sales-hook-sevrage.mjs --apply
 */

import { createClient } from "@supabase/supabase-js";
import { SALES_HOOKS_SEVRAGE } from "./data/sales-hook-sevrage.mjs";

const APPLY = process.argv.includes("--apply");
const SLUG = "je-souhaite-sevrer-mon-bebe";
...
for (const entry of SALES_HOOKS_SEVRAGE) {
...
console.log(`Correspondances trouvées : ${matched.length}/${SALES_HOOKS_SEVRAGE.length}\n`);
