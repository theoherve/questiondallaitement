 ...
 * « Je me prépare à allaiter » (accompagnement_sections.sales_hook).
 *
 ...
 * data/sales-hook-je-me-prepare.mjs. Un chapitre sans correspondance est
 * signalé, jamais deviné.
 *
 *   node --env-file=.env.local scripts/apply-sales-hook-je-me-prepare.mjs          # dry-run
 *   node --env-file=.env.local scripts/apply-sales-hook-je-me-prepare.mjs --apply
 */

import { createClient } from "@supabase/supabase-js";
import { SALES_HOOKS_JE_ME_PREPARE } from "./data/sales-hook-je-me-prepare.mjs";

const APPLY = process.argv.includes("--apply");
const SLUG = "je-me-prepare-a-allaiter";
...
for (const entry of SALES_HOOKS_JE_ME_PREPARE) {
...
console.log(`Correspondances trouvées : ${matched.length}/${SALES_HOOKS_JE_ME_PREPARE.length}\n`);
