 ...
 * « De la couveuse au sein » (accompagnement_sections.sales_hook).
 *
 ...
 * data/sales-hook-couveuse.mjs. Un chapitre sans correspondance est signalé,
 * jamais deviné.
 *
 *   node --env-file=.env.local scripts/apply-sales-hook-couveuse.mjs          # dry-run
 *   node --env-file=.env.local scripts/apply-sales-hook-couveuse.mjs --apply
 */

import { createClient } from "@supabase/supabase-js";
import { SALES_HOOKS_COUVEUSE } from "./data/sales-hook-couveuse.mjs";

const APPLY = process.argv.includes("--apply");
const SLUG = "de-la-couveuse-au-sein";
...
for (const entry of SALES_HOOKS_COUVEUSE) {
...
console.log(`Correspondances trouvées : ${matched.length}/${SALES_HOOKS_COUVEUSE.length}\n`);
