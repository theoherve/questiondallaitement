 ...
 * l'acces. La page serveur ne rend cet ilot que lorsque `?purchased=<id>` est
 * present et que l'inscription n'apparait pas encore : on sonde jusqu'a ce que
 * le webhook ait cree la ligne, puis `router.refresh()` re-rend la liste avec
...
export const PurchaseReconciler = ({
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (ready) {
        // Retire `?purchased` pour ne pas relancer le sondage au prochain rendu.
    ...
    >
      {timedOut ? (
        ...
        <>
          ...
          <span>Validation de votre paiement, votre accompagnement arrive…</span>
        </>
      )}
    </div>
  );
