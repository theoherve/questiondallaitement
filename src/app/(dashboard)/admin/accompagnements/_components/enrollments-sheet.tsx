...
import { unenrollFromAccompagnement } from "@/app/(dashboard)/admin/accompagnements/[id]/enroll-actions";
...
export const EnrollmentsSheet = ({
  ...
  const handleUnenroll = (enrollmentId: string) => {
    ...
    startTransition(async () => {
      const result = await unenrollFromAccompagnement(enrollmentId);
      if (result.success) {
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <SheetDescription className="mt-1 text-left">
                Inscrits à <strong>{accompagnementTitle}</strong>
              </SheetDescription>
            </div>
            <EnrollModal
              accompagnementId={accompagnementId}
              accompagnementTitle={accompagnementTitle}
              trigger={
                <Button size="sm">
