...
import {
  ...
  removeCollaborator,
  type AccompagnementCollaboratorRow,
} from "../actions";
...
export const CollaboratorManager = ({
  ...
  collaborators: AccompagnementCollaboratorRow[];
  ...
  const handleAdd = () => {
    ...
    startTransition(async () => {
      const result = await addCollaborator(accompagnementId, {
  ...
  const handleRemove = (consultantId: string) => {
    startTransition(async () => {
      const result = await removeCollaborator(accompagnementId, consultantId);
      if (result.success) {
