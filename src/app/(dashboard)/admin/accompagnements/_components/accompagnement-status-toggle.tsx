...
import { updateAccompagnementStatus } from "../actions";
...
export const AccompagnementStatusToggle = ({
  accompagnementId,
  currentStatus,
}: Props) => {
  ...
  const handleChange = async (newStatus: Status) => {
    ...
    setIsPending(true);
    const result = await updateAccompagnementStatus(accompagnementId, newStatus);
    setIsPending(false);
