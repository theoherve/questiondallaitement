...
import { AccompagnementStatusToggle } from "./_components/accompagnement-status-toggle";
import { DuplicateAccompagnementButton } from "./_components/duplicate-accompagnement-button";
...
const AdminAccompagnementsPage = async ({ searchParams }: Props) => {
  ...
  const { data: accompagnements, error: accompagnementsError } = await query;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button asChild className="bg-primary-red hover:bg-primary-red-dark">
          <Link href="/admin/accompagnements/nouveau" tabIndex={0}>
      ...
      {accompagnementsError && (
        <p className="rounded-md bg-destructive/10 p-3 text-sm text-destructive">
          Erreur lors du chargement des accompagnements : {accompagnementsError.message}
        </p>
      )}
            ...
            <TableBody>
              {rows.length === 0 ? (
                ...
                rows.map((accompagnement) => {
                  ...
                  return (
                    <TableRow key={accompagnement.id}>
                      <TableCell>
                        <Link
                          href={`/admin/accompagnements/${accompagnement.id}/edit`}
                      ...
                      <TableCell>{formatPrice(accompagnement.price_cents)}</TableCell>
                      <TableCell>
                        <AccompagnementStatusToggle
                          accompagnementId={accompagnement.id}
                          currentStatus={
                            (accompagnement.status as "draft" | "published" | "archived") ??
                            "draft"
                          }
                        />
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          <Button asChild variant="ghost" size="icon">
                            <Link
                              href={`/accompagnements/${accompagnement.slug}`}
                              target="_blank"
                              tabIndex={0}
                              aria-label={`Voir ${accompagnement.title} sur le site`}
                            >
                              <ExternalLink className="h-4 w-4" />
                            </Link>
                          </Button>
                          <DuplicateAccompagnementButton
                            accompagnementId={accompagnement.id}
                          ...
                          <Button asChild variant="ghost" size="icon">
                            <Link
                              href={`/admin/accompagnements/${accompagnement.id}/edit`}
                              tabIndex={0}
                              aria-label={`Modifier ${accompagnement.title}`}
                            >
...
export default AdminAccompagnementsPage;
