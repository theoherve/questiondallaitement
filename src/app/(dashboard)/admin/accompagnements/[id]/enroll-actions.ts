...
export const searchClientsForEnroll = async (
  ...
  const parsedAccompagnementId = uuidSchema.safeParse(accompagnementId);
  if (!parsedAccompagnementId.success) {
...
const insertEnrollmentAndNotify = async (args: {
  ...
  const root = baseUrl();
  const accompagnementUrl = `${root}/espace-client/accompagnements/${args.accompagnementId}`;
  ...
  // /connexion; they navigate to the accompagnement from /espace-client.
  let accessUrl = accompagnementUrl;
  ...
  try {
    ...
    console.error("[manualEnroll] sendAccompagnementAccess failed:", err);
  }
  ...
  revalidatePath(`/admin/accompagnements/${args.accompagnementId}/edit`);
...
const loadAccompagnementTitle = async (
  accompagnementId: string,
...
export const manualEnrollExistingClient = async (
  ...
  const parsedAccompagnement = uuidSchema.safeParse(accompagnementId);
  const parsedClient = uuidSchema.safeParse(clientId);
  if (!parsedAccompagnement.success || !parsedClient.success) {
    return { success: false, error: "Identifiants invalides" };
  }

  const accompagnement = await loadAccompagnementTitle(parsedAccompagnement.data);
  if (!accompagnement) {
  ...
  return insertEnrollmentAndNotify({
    ...
    accompagnementId: parsedAccompagnement.data,
...
export const manualEnrollNewClient = async (
  ...
  const parsedAccompagnement = uuidSchema.safeParse(accompagnementId);
  if (!parsedAccompagnement.success) {
    return { success: false, error: "Accompagnement invalide" };
  }
...
export type AvailableAccompagnement = {
  id: string;
  title: string;
  status: "draft" | "published" | "archived";
};

export const listAvailableAccompagnementsForClient = async (
  clientId: string,
): Promise<ActionResult<AvailableAccompagnement[]>> => {
  await requireEnrollRole();
  ...
  return { success: true, data: (data ?? []) as AvailableAccompagnement[] };
};

export const unenrollFromAccompagnement = async (
  enrollmentId: string,
